...
local function OnClick(self, button)
	if button == "RightButton" then
		local text = GetQuestLogTitle(self.ObeliskQuestClassic_TitleIdx)
		frame:SetZoneAndShow(text)
	else
...
local function RegisterHeaderClicks()
	local i = 1
	while _G["QuestLogExTitle" .. i] do
		local headerFrame = _G["QuestLogExTitle" .. i]

		if headerFrame.isHeader then
			if headerFrame then
				headerFrame.ObeliskQuestClassic_TitleIdx = i
				headerFrame:SetScript("OnClick", OnClick)
			end
		end
...
local function OnQuestLogToggle()
	if QuestLogExFrame:IsShown() then
		RegisterHeaderClicks()
	end
end

function frame:PLAYER_ENTERING_WORLD()
	hooksecurefunc("ToggleQuestLog", OnQuestLogToggle)
end
